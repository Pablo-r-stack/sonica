    ...
    try {
        ...
        if (datos.shows && datos.shows.length > 0) {
            datos.shows.forEach(show => {
                const nuevaCarta = crearCartaBanda(show.nombre, show.fecha, show.lugar, show.hora, show.direccion, show.mapa, show.imagen, show.descripcion);
...
function crearCartaBanda(nombre, fecha, lugar, hora, direccion, mapa, imagenSrc, descripcion) {
    ...
    nuevaCarta.querySelector('.carta').addEventListener('click', () => {
        localStorage.setItem('selectedShow', JSON.stringify({ nombre, fecha, lugar, hora, direccion, mapa, imagenSrc, descripcion }));
